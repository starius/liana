...
pub(crate) fn height_u32_from_i32(height: i32) -> u32 {
    height.try_into().expect("height must fit into u32")
}

pub(crate) fn height_i32_from_u32(height: u32) -> i32 {
    height.try_into().expect("height must fit into i32")
}

pub(crate) fn block_id_from_tip(tip: BlockChainTip) -> bdk_electrum::bdk_chain::BlockId {
    bdk_electrum::bdk_chain::BlockId {
        height: height_u32_from_i32(tip.height),
        hash: tip.hash,
    }
}

pub(crate) fn block_info_from_anchor(
    anchor: ConfirmationTimeHeightAnchor,
) -> crate::bitcoin::BlockInfo {
    crate::bitcoin::BlockInfo {
